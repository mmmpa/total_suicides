...
describe Tagged do
  describe 'scope' do
    describe 'year' do
      it { expect(Total.in([21, 22]).count).to eq(49 * 6) }
    ...
    describe 'area' do
      it { expect(Total.at([1, 2]).count).to eq(3 * 2 * 3) }
    ...
    describe 'gender' do
      it { expect(Total.by([1, 2]).count).to eq(3 * 49 * 2) }
  ...
  describe 'as_json' do
    it {expect(Day.in(21).at(1).by(2).first.to_json).not_to include('_id')}
    it {expect(Day.in(21).at(1).by(2).first.to_json).not_to include('"id"')}
  end
end