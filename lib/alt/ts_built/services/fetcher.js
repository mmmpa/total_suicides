...
var Fetcher = (function () {
    ...
    Fetcher.prototype.fetchRaw = function (params, callback) {
        var _this = this;
        var years = params.years, genders = params.genders, areas = params.areas, detail = params.detail;
        var gender = genders.join(',');
        var year = years.join(',');
        var area = areas.join(',');
        var uri = ['/api', gender, year, area, detail].join('/');
        request
            .get(uri)
...
exports.fetch = fetch;
function fetchRaw(params, callback) {
